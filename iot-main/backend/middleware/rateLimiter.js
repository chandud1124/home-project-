...
const authLimiter = rateLimit({
    ...
    max: process.env.RATE_LIMIT_MAX_REQUESTS || 1000, // 1000 requests per window
    message: {
    ...
    legacyHeaders: false,
    skip: (req) => false, // Enable rate limiting in all environments
    keyGenerator: (req) => {
        return req.ip + ':' + req.path; // Separate limits for each endpoint
    },
    handler: (req, res) => {
